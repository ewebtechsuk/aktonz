( function( $ ) {
	...
	var StickyTableHeader = /** @class */ (function () {
		...
		StickyTableHeader.prototype.listenResize = function() {
			if ( 'ResizeObserver' in window ) {
				...
				resizeObserver.observe( this.header.children[0].children[0] );
			}
		};
		...
		StickyTableHeader.prototype.createClone = function () {
			...
			this.headerSize = this.header.parentNode.getBoundingClientRect().width;

			this.updateCellsWidth();

			this.cloneContainer.style.display = 'table';
			this.cloneContainer.style.borderCollapse = 'inherit';
			this.cloneContainer.style.width = "".concat(this.headerSize, "px");
			this.cloneContainerParent.style.position = 'fixed';

			const isMobile = window.innerWidth <= 767;
			this.cloneContainerParent.style.overflow = isMobile ? 'hidden' : 'visible';

			this.cloneContainerParent.style.top = "".concat(this.getTop(), "px");
			this.setHorizontalScrollOnClone();
			return this.clone;
		};
		StickyTableHeader.prototype.updateCellsWidth = function () {
			if ( this.clone ) {
				Array.from(this.header.children).forEach( (row, rowIndex) => {
					Array.from(row.children).forEach( (cell, index) => {
						this.clone.children[rowIndex].children[index].style.width =
							(cell.getBoundingClientRect().width / this.headerSize) * 100 + '%';
					});
				});
			}
		};
		StickyTableHeader.prototype.setHorizontalScrollOnClone = function () {
			...
			if (isMobile) {
				...
				this.cloneContainerParent.scrollLeft = scrollLeft;
				if (this.cloneHeader) {
					this.cloneHeader.style.transform = `translateX(${-scrollLeft}px)`;
				}
			}
		};

		StickyTableHeader.prototype.handleResize = function () {
			const isMobile = window.innerWidth <= 767;
		...
		StickyTableHeader.prototype.refreshTable = function() {
			this.lastElement = null;
		};
		return StickyTableHeader;
	}());

	window.JetWCProductStickyTableHeader = StickyTableHeader;
